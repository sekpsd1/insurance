...
const SCLASS_LABELS: Record<string, string> = {
...
function getSClassLabel(value: string) {
  return SCLASS_LABELS[value] ?? value;
}
...
export default function SearchPremiumForm({
  ...
  const sClassOptions = useMemo(() => uniqueValues(optionRows.map((row) => row.sClass)), [optionRows]);
  ...
  const sumInsuredOptions = useMemo(() => {
    ...
    const values = filteredRows
      ...
      .flatMap((row) => [row.minSumInsured, row.maxSumInsured])
  ...
  const cubicCapacityOptions = useMemo(() => {
    ...
    filteredRows
      .filter((row) => row.brand === brand && row.model === model && row.years.includes(year))
      .forEach((row) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="mt-6 rounded-3xl bg-white p-5 shadow-[0_10px_30px_rgba(4,16,61,0.08)] ring-1 ring-white/70">
        ...
        <div>
          ...
          <div className="relative">
            <select
              ...
              className="w-full appearance-none rounded-2xl border border-[#d8dcec] bg-[#eaecf7] px-4 py-4 pr-12 text-[16px] text-[#12131a] outline-none transition disabled:cursor-not-allowed disabled:opacity-60 focus:border-[#0047BA] focus:bg-white focus:ring-4 focus:ring-[#0047BA]/10"
            >
              <option value="">{coverage ? '-- เลือกยี่ห้อรถ --' : 'กรุณาเลือกประเภทกรมธรรม์ก่อน'}</option>
              {brands.map((item) => (
                <option key={item} value={item}>
                  {item}
                </option>
      ...
      <section className="mt-6 rounded-2xl bg-[#dfe5ff] px-4 py-4 text-[#3a4258] shadow-[0_8px_24px_rgba(0,0,0,0.05)]">
        <div className="flex items-start gap-3">
          ...
          <p className="text-sm font-medium leading-6">
            ระบบจะกรองจากประเภทรถ ประเภทกรมธรรม์ อายุรถ และทุนประกันตามตารางเบี้ยของบริษัทประกัน
          </p>
        </div>
      </section>
      ...
      <div className="mt-6">
        <button
          type="submit"
          disabled={!sClass || !coverage || !brand || !model || !year || !cubicCapacity || !sumInsured}

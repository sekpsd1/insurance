...
- Policy Info page now follows the customer-supplied insurance notification Excel more closely:
  ...
  - Vehicle and policy document uploads now accept files up to 20MB while the overall Server Action request limit remains 25MB.
...
- Search Premium quote lead forms no longer display the connected LINE profile confirmation to customers, while still keeping the LINE user ID internally when LIFF profile data is available. In no-campaign quote mode, sum insured is optional so customers can request a quote even when no campaign sum-insured option exists.
- Quote lead submission now attempts to send a short `ขอใบเสนอราคา` message into the current LINE OA chat through LIFF after the lead is saved, including the lead number, customer contact, policy type, and vehicle summary, so admins can see which chat raised the quote request. If the LIFF chat-message permission is unavailable, the saved lead/email flow still completes normally.
...
- Added `/line-app/other-insurance` as a Rich Menu destination for other insurance types. It currently displays disabled Home, Personal Accident, and Travel Insurance buttons until their final destinations are provided, and accepts both direct Mini App and `liff.state` routes.
- Tracking lookup and tracking detail pages now include a `กลับไปเมนู` action that returns customers to the in-app main menu.
- Tracking lookup and tracking detail pages now use the same blue top header pattern as other customer pages, with the return-to-menu action in the header and a plain link fallback for more reliable LINE webview navigation.
- Success page secondary action now says `กลับไปเมนู` and returns customers to the in-app main menu instead of the package search page.
...
- Compare page displays two selected packages side by side as a detailed coverage-comparison table, including third-party liability, own-damage/lost-fire coverage, attached-document benefits, premium, CTP, total, and remaining payable rows; the desktop table uses section icons, colored plan columns, highlighted totals, and a more prominent remaining-payable row while keeping remove/cart actions in the plan headers. The mobile view now uses a two-column vertical layout so customers scroll up/down to read all coverage items instead of swiping horizontally, keeps the comparison heading on one line, clamps long insurer names in the plan header, shows the full 3.1 land-vehicle collision wording for type 2+ and 3+ plans, and uses each package's own passenger/medical seat count instead of reusing the first plan's value.
- Compare page footer buttons now use clearer labels: `กลับไปหน้าเมนู` and `กลับไปเลือกแผนประกัน`.
...
- Cart plan cards now match the compact results-card layout more closely, including the full-width brand/model/year title, smaller icons, tighter spacing, compact cost summary, and compact remaining-payable section. The cart header only shows the number of saved plans and no longer shows a combined remaining-payable total. Opening `/line-app/cart` without query IDs now reads saved cart IDs from localStorage and redirects to the populated cart URL, so Rich Menu cart links can show the customer's saved plans. Clearing the cart now clears cart/compare localStorage and replaces the URL with clean `/line-app/cart`; if the user goes back to an old cart URL after clearing, the page redirects back to the empty cart instead of restoring stale `ids`.
- Cart now stores the originating results/search query alongside saved package IDs and has a left-side `กลับไปเมนู` LIFF close button in the cart header, so customers opening the cart from Rich Menu can close back to LINE or return to the same offer/results page with `กลับไปเลือกแผน`. If LIFF cannot close the webview, the button falls back to the LINE OA link.
...
- Customer Policy Info vehicle section now shows the brand, model, size, and registration year selected during premium search as read-only `รายละเอียดรถยนต์` fields, so customers do not need to type these values again. The selected values are still submitted and stored on the order, including `carCubicCapacity`, for admin, provider Magic Link, tracking, email preview, and CSV export.

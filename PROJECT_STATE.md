...
- Cart now stores the originating results/search query alongside saved package IDs and has a `กลับไปเมนู` LIFF close button in the cart header, so customers opening the cart from Rich Menu can close back to LINE or return to the same offer/results page with `กลับไปเลือกแผน`.

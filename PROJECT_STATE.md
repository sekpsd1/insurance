...
- Search Premium `กลับเมนู LINE` now initializes LIFF with external-browser login disabled before calling `closeWindow`, so the button can close the Mini App while avoiding unwanted LINE auth redirects to `access.line.me`.
